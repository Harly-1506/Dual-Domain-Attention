...
class RAFDB_Trainer_KFold(Trainer):
  def __init__(self, model, train_loader, val_loader, test_loader,test_loader_ttau, configs, wb = True):
    ...
    self.kfold = KFold(n_splits=self.k_folds, shuffle=True)
    # load dataset
...
#       loss += l2
  ...
  def step_per_val(self):
    ...
    #   self.val_acc_list.append(val_acc / i)
      ...
      print(" Val_Loss: {:.4f}".format(val_loss / i),", Val_Accuracy: {:.2f}%".format(val_acc / i))
  ...
  def Train_model(self):
    ...
    try:
      while not self.stop_train():
        ...
        for fold, (train_ids, test_ids) in enumerate(self.kfold.split(self.train_loader)):
            print(f'FOLD {fold}')
            print(len(train_ids))
            print(len(test_ids))
            val_acc = 0
            ...
            self.val_ds = DataLoader(self.train_loader, batch_size = self.batch_size, num_workers=self.num_workers,
                    pin_memory=True,  sampler=test_subsampler)

            self.step_per_train()
            val_acc , val_loss = self.step_per_val()
            self.results_acc[fold] = val_acc
            self.results_loss[fold] = val_loss

        sum_acc = 0.0
        sum_loss = 0.0
        for key, value in self.results_acc.items():
            sum_acc += value
        for key, value in self.results_loss.items():
            sum_loss += value
        # print(f'Average: {sum/len(results.items())} %')

        self.val_loss_list.append(sum_loss/len(self.results_acc.items()))
        self.val_acc_list.append(sum_acc/len(self.results_acc.items()))
        print(" Final_Val_Loss: {:.4f}".format(self.val_loss_list[-1]),", Final_Val_Accuracy: {:.2f}%".format(self.val_acc_list[-1]))
